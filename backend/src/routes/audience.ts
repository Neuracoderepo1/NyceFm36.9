...
audienceRouter.post("/requests",publicLimiter,async(req,res)=>{const requestSchema=actor.merge(z.object({stationId:station,mediaAssetId:z.string().uuid().optional(),title:z.string().min(1).max(200).optional(),artist:z.string().max(200).optional(),message:z.string().max(500).optional()})); const b=requestSchema.refine(x=>x.mediaAssetId||x.title,{message:'mediaAssetId or title is required'}).parse(req.body??{});const id=await stationIdOrDefault(b.stationId);const a=actorFor(req,b);res.status(201).json({request:await createRequest({stationId:id,actor:a,mediaAssetId:b.mediaAssetId,title:b.title,artist:b.artist,message:b.message})});});
